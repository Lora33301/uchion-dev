...
export class KidsSlideGenerator extends BaseSlideGenerator {
  protected readonly HEADING_FONT = 'Arial'
  ...
  protected readonly contentAccentColor = COLORS.teal
  protected readonly mutedColor = COLORS.muted

  private readonly WM_COLOR = 'D0C9B5'

  // --- Decorative helpers ---

  private addDecoCircles(pres: PptxGenJS, slide: PptxGenJS.Slide): void {
    const circles = [
  ...
  protected addTitleSlide(
    ...
    if (category) {
      s.addText(category.toUpperCase(), {
        ...
        fontSize: 12, fontFace: this.BODY_FONT, color: COLORS.teal,
    ...
    s.addText(slide.title, {
      ...
      fontSize: 38, fontFace: this.BODY_FONT, color: COLORS.text,
    ...
    if (subtitle) {
      s.addText(subtitle, {
        ...
        fontSize: 16, fontFace: this.BODY_FONT, color: COLORS.muted,
      })
    }
    ...
    this.addWatermark(s, this.WM_COLOR)
  }
  ...
  protected addContentSlide(
    ...
    if (sectionNum) {
      s.addText(sectionNum, {
        ...
        fontSize: 12, fontFace: this.BODY_FONT, color: COLORS.teal, bold: true,
      })
    }

    s.addText(slide.title, {
      x: 0.7, y: sectionNum ? 0.8 : 0.5, w: 8.6, h: 0.7,
      fontSize: 26, fontFace: this.BODY_FONT, color: COLORS.text,
      bold: true,
    })
  ...
  protected addTwoColumnSlide(
    ...
    if (sectionNum) {
      s.addText(sectionNum, {
        ...
        fontSize: 12, fontFace: this.BODY_FONT, color: COLORS.white,
        bold: true,
      })
    }
    ...
    s.addText(slide.title, {
      ...
      fontSize: 26, fontFace: this.BODY_FONT, color: COLORS.white,
  ...
  protected addTableSlide(
    ...
    s.addText(slide.title, {
      ...
      fontSize: 26, fontFace: this.BODY_FONT, color: COLORS.text, bold: true,
    })
    ...
    if (td && td.headers.length > 0) {
      const headerRow: PptxGenJS.TableCell[] = td.headers.map(h => ({
        text: h,
        options: {
          bold: true, fontSize: 14, fontFace: this.BODY_FONT,
  ...
  protected addFormulaSlide(
    ...
    s.addText(formula, {
      ...
      fontSize: 40, fontFace: this.BODY_FONT, color: COLORS.teal,
    ...
    if (description) {
      s.addText(description, {
        ...
        fontSize: 13, fontFace: this.BODY_FONT, color: COLORS.muted,
        align: 'center',
      })
    }
  ...
  protected addEndSlide(
    ...
    s.addText('МОЛОДЦЫ!', {
      ...
      fontSize: 14, fontFace: this.BODY_FONT, color: COLORS.teal,
    ...
    s.addText(slide.title || 'Вопросы?', {
      ...
      fontSize: 44, fontFace: this.BODY_FONT, color: COLORS.text,
      bold: true, align: 'center',
    })
